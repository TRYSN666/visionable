...
export async function parsePortsXlsx(xlsxBytes: Buffer, sourceFile = "ports.xlsx"): Promise<TopologySnapshot> {
  if (xlsxBytes.byteLength > MAX_IMPORT_SIZE) throw new PortsCsvError("连线表超过 12MiB 限制");
  if (xlsxBytes.byteLength === 0) throw new PortsCsvError("XLSX 内容不能为空");
  try {
    const rows = await readSheet(Readable.from([xlsxBytes]));
    if (rows.length === 0) throw new PortsCsvError("连线表没有数据行");
    return parsePortsCsv(rows.map((row) => row.map(csvCell).join(",")).join("\n"), sourceFile);
  } catch (error) {
    if (error instanceof PortsCsvError) throw error;
...
function findHeader(headers: string[], aliases: string[], required: boolean): string | undefined {
  const normalizedAliases = new Set(aliases.map(normalizeHeader));
  const result = headers.find((header) => normalizedAliases.has(normalizeHeader(header)));
  if (!result && required) throw new PortsCsvError(`缺少必需字段：${aliases[0]}`);
  return result;
}

export function inferImportedRole(hostname: string): DeviceRole {
  const value = hostname.toUpperCase();
  if (/(?:^|-)CSW(?:-?\d+)?(?:-|$)/.test(value)) return "CSW";
  if (/(?:^|-)SSW(?:-?\d+)?(?:-|$)/.test(value) || /(?:^|-)SPINE(?:-?\d+)?(?:-|$)/.test(value)) return "SSW";
  if (/(?:^|-)ASW(?:-?\d+)?(?:-|$)/.test(value)) return "ASW";
  if (/(?:^|-)MSW(?:-?\d+)?(?:-|$)/.test(value)) return "MGMT";
  if (/(?:^|-)OAW(?:-?\d+)?(?:-|$)/.test(value)) return "OOB";
  if (/(?:^|-)IBCR\d*(?:-|$)/.test(value)) return "IBCR";
  if (/(?:^|-)IBSP\d*(?:-|$)/.test(value)) return "IBSP";
  if (/(?:^|-)IBLF\d*(?:-|$)/.test(value)) return "IBLF";
  if (/(?:^|-)GPU(?:-|$)/.test(value)) return "ENDPOINT";
  if (
    value.includes("UFM") ||
    value === "HPN" ||
...
function nodeId(hostname: string): string {
...
export function parsePortsCsv(csvText: string, sourceFile = "ports.csv"): TopologySnapshot {
  ...
  const interfaces = new Map<string, Map<string, InterfaceRecord>>();
  const links = new Map<string, ImportedLink>();

  const getNode = (hostname: string): TopologyNode => {
  ...
  const addNodeLid = (hostname: string, lid: string): void => {
    if (!lid) return;
    const node = getNode(hostname);
    if (!node.lids?.includes(lid)) node.lids?.push(lid);
  };

  const getInterface = (hostname: string, port: string): InterfaceRecord => {
    const key = hostname.toLowerCase();
    const node = getNode(hostname);
  ...
  const addInterfaceAddress = (record: InterfaceRecord, cidr: string): void => {
    if (!cidr || record.addresses.some((address) => address.cidr === cidr)) return;
  ...
  for (const [index, row] of rows.entries()) {
    ...
    else links.set(key, { key, sourceHostname: localHost, sourcePort: localPort, targetHostname: peerHost, targetPort: peerPort, count: 1 });
  }
  ...
  for (const node of nodes.values()) {
    ...
    if (node.lids?.length === 0) delete node.lids;
  }
  const topologyLinks: TopologyLink[] = [...links.values()].map((link) => ({
