...
export function inferRole(hostname: string): DeviceRole {
  const match = hostname.match(ROLE_PATTERN);
  return (match?.[1]?.toUpperCase() as DeviceRole | undefined) ?? "UNKNOWN";
}

export function inferPod(hostname: string): PodName {
  const match = hostname.match(POD_PATTERN);
  return match ? (`POD${match[1]}` as PodName) : "SHARED";
}

export function inferRack(hostname: string): string | undefined {
  return hostname.match(RACK_PATTERN)?.[1]?.toUpperCase();
}

export function inferEndpointType(hostname: string): string {
  const value = hostname.toUpperCase();
  if (value.includes("GPU")) return "GPU";
  if (/(?:^|-)BMC(?:-|$)/.test(value)) return "BMC";
  if (value.includes("FW")) return "防火墙";
  if (value.includes("DDN") || value.includes("STOR")) return "存储";
  if (value.includes("UFM")) return "UFM";
  if (value.startsWith("CON")) return "控制台";
  if (value.includes("SERVER") || value.includes("SRV")) return "服务器";
  return "其他终端";
}

function compactLabel(hostname: string, role: DeviceRole, pod: PodName): string {
  const match = hostname.match(ROLE_PATTERN);
  const sequence = match?.[2]?.padStart(3, "0");
  if (match && sequence) return pod.startsWith("POD") ? `${pod}-${role}-${sequence}` : `${role}-${sequence}`;
  return hostname.length > 28 ? `${hostname.slice(0, 25)}…` : hostname;
}

function isExplicitInterface(name: string): boolean {
  if (name.includes(",")) return false;
  if (/^swp\d+-\d+$/i.test(name)) return false;
  if (/^swp\d+s\d+-\d+$/i.test(name)) return false;
  return !/\s/.test(name);
}

function getOrCreateInterface(interfaces: Map<string, InterfaceRecord>, name: string): InterfaceRecord {
  const existing = interfaces.get(name);
  if (existing) return existing;
  const value: InterfaceRecord = { name, addresses: [], peers: [] };
  interfaces.set(name, value);
  return value;
}

export function parseNvueDocument(content: string, _sourceFile: string): ParsedDevice {
  void _sourceFile;
  const hostname = content.match(/^nv set system hostname (\S+)\s*$/m)?.[1];
  if (!hostname) throw new NvueParseError("未找到 NVUE system hostname");

  const interfaces = new Map<string, InterfaceRecord>();
  const links: DeclaredLink[] = [];

  for (const match of content.matchAll(/^nv set interface (\S+) description (.+?)\s*$/gm)) {
    const [, name, description] = match;
    if (!isExplicitInterface(name)) continue;
  ...
  const sortedInterfaces = [...interfaces.values()].sort((a, b) =>
...
function deviceId(hostname: string): string {
  return `device:${hostname.toLowerCase()}`;
}

function externalId(hostname: string): string {
...
function endpointKey(hostname: string, interfaceName: string): string {
  return `${hostname.toLowerCase()}\u0000${interfaceName.toLowerCase()}`;
}

function inferPlane(sourceRole: DeviceRole, targetRole: DeviceRole, sourceInterface: string): LinkPlane {
...
function buildClusters(nodes: TopologyNode[]): EndpointCluster[] {
  ...
  for (const [key, bucket] of [...buckets.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    const [pod, rack, endpointType] = key.split("|") as [PodName, string, string];
    for (let offset = 0; offset < bucket.length; offset += MAX_CLUSTER_SIZE) {
      ...
      for (const node of chunk) node.clusterId = id;
      clusters.push({
        id,
...
export async function parseConfigDirectory(configDir: string): Promise<TopologySnapshot> {
  ...
  if (devices.length === 0) throw new NvueParseError("配置目录中没有可用的 NVUE 设备配置");

  const configuredByHostname = new Map(devices.map((item) => [item.node.hostname.toLowerCase(), item]));
  const externalByHostname = new Map<string, TopologyNode>();
  const physicalLinks = new Map<string, { declarations: DeclaredLink[] }>();
  ...
  for (const [key, value] of physicalLinks) {
    ...
    const reciprocal = value.declarations.some(
      (item) =>
        item.sourceHostname.toLowerCase() === declaration.targetHostname.toLowerCase() &&
