...
import { ForwardingWorkbookError } from "./forwardingWorkbookParser.js";
import {
  CatalogNotFoundError,
  ForwardingSnapshotIncompatibleError,
  ForwardingSnapshotNotFoundError,
  InvalidLayoutNodeError,
  ServerNodeNotFoundError,
  TopologyStore,
} from "./store.js";
...
export function createApp({ store, serveClient = false }: AppOptions) {
  ...
  const catalogError = (error: unknown): { status: number; message: string } => {
    if (error instanceof CatalogNotFoundError) return { status: 404, message: error.message };
    if (error instanceof Error && /UNIQUE constraint failed/i.test(error.message)) return { status: 409, message: "名称已存在" };
    return { status: 400, message: error instanceof Error ? error.message : "操作失败" };
  };

  const parsePositions = (rawPositions: unknown): { positions: Record<string, { x: number; y: number }> } | { error: string } => {
    if (!rawPositions || typeof rawPositions !== "object" || Array.isArray(rawPositions)) return { error: "positions 必须是节点坐标对象" };
    const entries = Object.entries(rawPositions as Record<string, unknown>);
    if (entries.length === 0 || entries.length > 5000) return { error: "布局必须包含 1 至 5000 个节点" };
    const positions = Object.create(null) as Record<string, { x: number; y: number }>;
  ...
  app.post("/api/projects", (request, response) => {
    ...
    if (!validName(name)) return void response.status(400).json({ error: "项目名称必须为 1 至 80 个字符" });
    try {
  ...
  app.put("/api/projects/:projectId/topologies/:topologyId", (request, response) => {
    const { name } = request.body as { name?: unknown };
    if (!validName(name)) return void response.status(400).json({ error: "拓扑名称必须为 1 至 80 个字符" });
    try {
  ...
  app.post("/api/projects/:projectId/topologies/import", async (request, response) => {
    const { name, fileName, csvText, xlsxBase64 } = request.body as { name?: unknown; fileName?: unknown; csvText?: unknown; xlsxBase64?: unknown };
    if (!validName(name)) return void response.status(400).json({ error: "拓扑名称必须为 1 至 80 个字符" });
    if (typeof fileName !== "string" || fileName.length === 0 || fileName.length > 240 || !/\.(?:csv|xlsx)$/i.test(fileName)) return void response.status(400).json({ error: "仅支持 CSV 或 XLSX 连线表" });
    const isXlsx = fileName.toLowerCase().endsWith(".xlsx");
    let fileContent: string | Buffer;
    if (isXlsx) {
      if (typeof xlsxBase64 !== "string" || xlsxBase64.length === 0) return void response.status(400).json({ error: "XLSX 内容不能为空" });
      if (xlsxBase64.length > 16 * 1024 * 1024 + 16 || !/^[A-Za-z0-9+/]*={0,2}$/.test(xlsxBase64)) return void response.status(400).json({ error: "XLSX 内容编码无效或文件超过 12 MiB" });
      fileContent = Buffer.from(xlsxBase64, "base64");
    } else {
      if (typeof csvText !== "string" || csvText.length === 0) return void response.status(400).json({ error: "CSV 内容不能为空" });
      fileContent = csvText;
    }
    try {
  ...
  const forwardingUpload = express.raw({
    type: ["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/octet-stream"],
    limit: "24mb",
  });

  app.get("/api/projects/:projectId/topologies/:topologyId/forwarding-snapshots", (request, response) => {
    response.setHeader("Cache-Control", "no-store");
    try {
      const snapshots = store.forwardingSnapshots(request.params.projectId, request.params.topologyId);
      response.status(200).json({ snapshots, defaultSnapshotId: snapshots.find((item) => item.isDefault && item.compatible)?.id });
    } catch (error) {
      const result = catalogError(error);
      response.status(result.status).json({ error: result.message });
    }
  ...
  app.post("/api/projects/:projectId/topologies/:topologyId/forwarding-snapshots/:snapshotId/traces", (request, response) => {
    const body = request.body as Record<string, unknown>;
    if (typeof body.sourceDeviceId !== "string" || !body.sourceDeviceId || typeof body.sourceIp !== "string" || isIP(body.sourceIp) !== 4 || typeof body.destinationIp !== "string" || isIP(body.destinationIp) !== 4) {
      return void response.status(400).json({ error: "sourceDeviceId、sourceIp 和 destinationIp 均为必填项，地址仅支持 IPv4" });
    }
    if (body.sourceInterface !== undefined && (typeof body.sourceInterface !== "string" || body.sourceInterface.length > 160)) return void response.status(400).json({ error: "sourceInterface 无效" });
    if (body.vrf !== undefined && (typeof body.vrf !== "string" || body.vrf.length > 160)) return void response.status(400).json({ error: "vrf 无效" });
    try {
      const trace = store.traceForwarding(request.params.projectId, request.params.topologyId, request.params.snapshotId, {
        sourceDeviceId: body.sourceDeviceId,
        sourceIp: body.sourceIp,
        destinationIp: body.destinationIp,
        sourceInterface: body.sourceInterface as string | undefined,
        vrf: body.vrf as string | undefined,
      });
      response.status(200).json(trace);
    } catch (error) {
      const status = error instanceof ForwardingSnapshotNotFoundError ? 404 : error instanceof ForwardingSnapshotIncompatibleError ? 409 : error instanceof CatalogNotFoundError ? 404 : 400;
      response.status(status).json({ error: error instanceof Error ? error.message : "无法执行流量仿真" });
    }
  });

  app.put("/api/topology/layout", async (request, response) => {
    response.setHeader("Cache-Control", "no-store");
    const parsed = parsePositions((request.body as { positions?: unknown })?.positions);
    if ("error" in parsed) return void response.status(400).json({ error: parsed.error });
    try {
  ...
  app.put("/api/server-addresses/:hostname", async (request, response) => {
    response.setHeader("Cache-Control", "no-store");
    const { inBandIp, outOfBandIp } = request.body as Record<string, unknown>;
