...
function sourceInterfaces(topology: TopologySnapshot, data: ForwardingSnapshotData, request: ForwardingTraceRequest): SourceContext[] {
  ...
  let candidates: SourceContext[] = matchingTopologyInterfaces.flatMap((item) => {
...
export function traceForwarding(topologyId: string, snapshotId: string, topology: TopologySnapshot, data: ForwardingSnapshotData, request: ForwardingTraceRequest): ForwardingTraceResult {
  ...
  const packet: AbstractPacket = { innerSourceIp: request.sourceIp, innerDestinationIp: request.destinationIp, vrf: sourceContext?.vrf ?? request.vrf ?? DEFAULT_VRF, vlan: sourceContext?.vlan };
  const rootWork: WorkState = { deviceId: sourceNode.id, ingressInterface: sourceContext?.name ?? request.sourceInterface, packet };
  ...
  if (sourceContext?.row) evidence(rootStateId, "接口属性", sourceContext.row, rootSummary, sourceContext.sourceFile);
  ...
  while (queue.length > 0) {
    ...
    if (packetAtDevice.vlan !== undefined) {
      ...
      if (!bridgeMac) {
        const arp = data.arpEntries.find((entry) => {
          if (entry.deviceId !== item.work.deviceId || norm(entry.vrf) !== norm(vrf) || entry.ip !== packetAtDevice.innerDestinationIp || entry.status === "incomplete") return false;
    ...
    if (publicState.kind !== "decapsulate") publicState.kind = "lookup";
