...
import type { ServerAddressInfo, TopologySnapshot, TopologyWarning } from "../shared/topology.js";
import type { ServerAddressOverride } from "./serverAddressStore.js";
...
export interface ServerInventoryResult {
  records: Map<string, ServerInventoryRecord>;
...
function score(record: ServerInventoryRecord): number {
  return (record.consistent ? 100 : 0) + (record.inBandIp ? 10 : 0) + (record.outOfBandIp ? 10 : 0);
}

export async function loadServerInventory(configDir: string): Promise<ServerInventoryResult> {
  ...
  if (!source) return { records: new Map(), warnings };
  ...
  const records = new Map<string, ServerInventoryRecord>();
  for (const [index, row] of rows.entries()) {
    ...
    const record: ServerInventoryRecord = {
      hostname,
    ...
    if (existing) {
      ...
      if (score(record) > score(existing)) records.set(key, record);
    } else {
...
export function applyServerInventory(
  snapshot: TopologySnapshot,
  inventory: ServerInventoryResult,
  overrides: Map<string, ServerAddressOverride>,
): TopologySnapshot {
  let matched = 0;
  let manualOverrides = 0;
  const matchedInventory = new Set<string>();
  const nodes = snapshot.nodes.map((node) => {
    if (node.kind !== "external" || node.endpointType !== "GPU") return node;
    const key = node.hostname.toLowerCase();
    const record = inventory.records.get(key);
    const override = overrides.get(key);
    if (!record && !override) return node;
    if (record) {
      matched += 1;
      matchedInventory.add(key);
    }
    if (override) manualOverrides += 1;
    const serverInfo: ServerAddressInfo = {
  ...
  const unmatchedWarnings: TopologyWarning[] = [];
