...
export function ForwardingSimulator({ projectId, topologyId, topology, simulationDevice, onClose, onSimulationChange }: Props) {
  ...
  const startTrace = async () => {
    ...
    try {
      const response = await fetch(`${snapshotApi(projectId, topologyId)}/${encodeURIComponent(snapshotId)}/traces`, {
        ...
        body: JSON.stringify({ sourceDeviceId: simulationDevice.deviceId, sourceIp: simulationDevice.sourceIp, destinationIp }),
      });
  ...
  return (
    <aside className="forwarding-panel" aria-label="流量路径仿真">
      ...
      <div className="simulation-mode-actions">
        ...
        <button className="forwarding-start" type="button" disabled={busy} onClick={() => {
          setError(undefined);
          if (!simulationDevice) setError("请在拓扑设备上点击右键，创建模拟设备并填写源 IP");
          else if (!snapshotId) setError("请先导入转发表");
          else setStartDialogOpen(true);
        }}><Play size={15} /><span>开始仿真</span></button>
      ...
      <div className="simulation-mode-guide">
        <b>操作提示</b>
        <p>右键拓扑上的任意设备，创建模拟设备并填写源 IP；开始时再填写目的 IP。</p>
      ...
      <div className="simulation-status-grid">
        ...
        <div><span>模拟设备</span><b>{selectedDevice?.hostname ?? "尚未创建"}</b><small>{simulationDevice ? `源 IP ${simulationDevice.sourceIp}` : "请在画布设备上点击右键"}</small></div>
      </div>
      ...
      {startDialogOpen && createPortal(
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setStartDialogOpen(false); }}>
          <form className="simulation-start-modal" role="dialog" aria-modal="true" aria-labelledby="simulation-start-title" onSubmit={(event) => { event.preventDefault(); void startTrace(); }}>
            <p>开始仿真</p>
            <h2 id="simulation-start-title">设置本次流量目的地址</h2>
