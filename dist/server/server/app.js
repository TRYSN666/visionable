...
import { CatalogNotFoundError, ForwardingSnapshotIncompatibleError, ForwardingSnapshotNotFoundError, InvalidLayoutNodeError, ServerNodeNotFoundError, } from "./store.js";
export function createApp({ store, serveClient = false }) {
        ...
        try {
            ...
            if (Buffer.isBuffer(request.body)) {
                ...
                if (!Array.isArray(files) || files.length === 0 || files.length > 50)
                    return void response.status(400).json({ error: "请选择 1 至 50 个 XLSX 转发表文件" });
                let totalBytes = 0;
                const workbooks = [];
                for (const file of files) {
                    ...
                    if (xlsxBytes.byteLength === 0 || totalBytes > 24 * 1024 * 1024)
                        return void response.status(400).json({ error: "转发表文件总大小不能超过 24 MiB" });
    ...
    app.post("/api/projects/:projectId/topologies/:topologyId/forwarding-snapshots/:snapshotId/traces", (request, response) => {
        ...
        if (body.sourceInterface !== undefined && (typeof body.sourceInterface !== "string" || body.sourceInterface.length > 160))
            return void response.status(400).json({ error: "sourceInterface 无效" });
        if (body.vrf !== undefined && (typeof body.vrf !== "string" || body.vrf.length > 160))
            return void response.status(400).json({ error: "vrf 无效" });
        try {
