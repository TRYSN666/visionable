...
import type { ForwardingSnapshotData, ForwardingSnapshotSummary, ForwardingTraceResult } from "../shared/forwarding";
...
const snapshotData: ForwardingSnapshotData = {
  metadata: { templateVersion: "1.0", batchName: snapshot.batchName, collectedAt: snapshot.collectedAt },
  interfaces: [
    { row: 2, deviceId: "a", deviceName: "SW-A", interfaceName: "eth0", interfaceType: "physical", forwardingMode: "access", vrf: "default", vlan: 1000, allowedVlans: [], status: "up" },
    { row: 3, deviceId: "a", deviceName: "SW-A", interfaceName: "vlan1000", interfaceType: "svi", forwardingMode: "routed", vrf: "magaspeed", vlan: 1000, allowedVlans: [], status: "up" },
    { row: 4, deviceId: "b", deviceName: "SW-B", interfaceName: "eth0", interfaceType: "physical", forwardingMode: "routed", vrf: "default", allowedVlans: [], status: "up" },
  ],
  lagMembers: [],
  routes: [],
  arpEntries: [{ row: 2, deviceId: "a", deviceName: "SW-A", vrf: "magaspeed", ip: "10.0.0.1", mac: "00:11:22:33:44:55", interfaceName: "vlan1000", status: "reachable" }],
  macEntries: [],
  vxlanEntries: [],
};
...
describe("ForwardingSimulator", () => {
  it("loads a snapshot, traces a path in reduced-motion mode and clears the simulation", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      if (init?.method === "POST") return jsonResponse(trace);
      if (String(input).endsWith(`/${snapshot.id}`)) return jsonResponse(snapshotData);
      return jsonResponse({ snapshots: [snapshot], defaultSnapshotId: snapshot.id });
    });
    ...
    const injectionPort = await screen.findByRole("combobox", { name: "流量注入端口" });
    ...
    expect(await screen.findByText("请选择流量注入端口")).toBeInTheDocument();
    fireEvent.change(injectionPort, { target: { value: "eth0" } });
    fireEvent.click(screen.getByRole("button", { name: "确认开始" }));
    ...
    const traceRequest = fetchMock.mock.calls.find(([, init]) => init?.method === "POST")?.[1];
    expect(JSON.parse(String(traceRequest?.body))).toMatchObject({ sourceDeviceId: "a", sourceIp: "10.0.0.1", destinationIp: "10.0.0.2", sourceInterface: "eth0", vrf: "magaspeed" });
